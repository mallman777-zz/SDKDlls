// MFCLibrary.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "MFCLibrary.h"


#ifdef _DEBUG
#define new DEBUG_NEW
#endif

//
//TODO: If this DLL is dynamically linked against the MFC DLLs,
//		any functions exported from this DLL which call into
//		MFC must have the AFX_MANAGE_STATE macro added at the
//		very beginning of the function.
//
//		For example:
//
//		extern "C" BOOL PASCAL EXPORT ExportedFunction()
//		{
//			AFX_MANAGE_STATE(AfxGetStaticModuleState());
//			// normal function body here
//		}
//
//		It is very important that this macro appear in each
//		function, prior to any calls into MFC.  This means that
//		it must appear as the first statement within the 
//		function, even before any object variable declarations
//		as their constructors may generate calls into the MFC
//		DLL.
//
//		Please see MFC Technical Notes 33 and 58 for additional
//		details.
//

// CMFCLibraryApp

BEGIN_MESSAGE_MAP(CMFCLibraryApp, CWinApp)
END_MESSAGE_MAP()


// CMFCLibraryApp construction

CMFCLibraryApp::CMFCLibraryApp()
{
	// TODO: add construction code here,
	// Place all significant initialization in InitInstance
}


// The one and only CMFCLibraryApp object

CMFCLibraryApp theApp;


// CMFCLibraryApp initialization

BOOL CMFCLibraryApp::InitInstance()
{
	CWinApp::InitInstance();

	return TRUE;
}




double localMath::localAdder(double x, double y) {
	return x + y;
}

double localMath::localMultiplier(double x, double y) {
	return x*y;
}

double newAdd(double a, double b)
{
	localMath lM;
	double outVar = lM.localAdder(a, b);
	return outVar;
}

double Multiply(double a, double b)
{
	return a * b;
}

double AddMultiply(double a, double b)
{
	return a + (a * b);
}

long connectToSA() {
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	myClass NrkSdk;
	if (!NrkSdk.CreateDispatch(_T("SpatialAnalyzerSDK.Application")))
	{
		AfxMessageBox(_T("Something Failed in OnCreate.  Added Macro"));
		return -1;  // fail
	}
	long statCode;
	CString host = _T("localhost");
	statCode = NrkSdk.Connect(host);
	return statCode;
}